...
void serialConfig(){
    ...
    if(texto1.equals("ls"))
    ...
    else if(texto1.equals("write")){
      Serial.println("O que deseja escrever em " + texto2);
      while(Serial.available()<=0){
      }
      writeFile(SD, caminho +texto2,Serial.readString());
    }
    else if(texto1.equals("rnfile")){
      Serial.println("Qual novo nome de " + texto2);
      while(Serial.available()<=0){
      }
      renameFile(SD, caminho + texto2,caminho + Serial.readString());
    }
      ...
      appendFile(SD, caminho +texto2,Serial.readString());
    }
